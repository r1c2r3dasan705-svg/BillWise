...
try {
    ...
    // Eliminar respostas e tópicos do fórum associados ao utilizador
    $stmt = $pdo->prepare("DELETE FROM respostas WHERE utilizador_id = ?");
    $stmt->execute([$user_id]);

    $stmt = $pdo->prepare("DELETE FROM topicos WHERE utilizador_id = ?");
    $stmt->execute([$user_id]);
